...
static const char vi_version[] = "vi v2.0 2018/06/01 08:06:31";
...
static int screensize; // screen buffer size
...
#define TES_CM      "\033[%d;%dH"
// move cursor up one line
#define TES_UP      "\033[A"
// move cursor down one line
#define TES_DOWN    "\n"
// Clear from cursor to end of line
#define TES_CEOL    "\033[0K"
// Clear from cursor to end of screen
#define TES_CEOS    "\033[0J"
// Terminal standout mode on
#define TES_TSM_ON  "\033[7m"
// Terminal standout mode off
#define TES_TSM_OFF "\033[0m"
...
int main(int argc, char **argv)
{
    ...
    // The argv array can be used by the ":next"
    // and ":rewind" commands save optind.
...
static void edit_file(Byte *fn)
{
    ...
    if(fn != 0)
    {
        // read text content to the memory
...
//----------------------------- The ASCII chart table -----------------------------
//
// SOH: start of headline      VT: vertical tab
// STX: start of text          FF: NP form feed, new page
// ETX: end of text            BS: backspace
// EOT: end of transmission    HT: horizontal tab
// ENQ: enquiry                LF: NL line feed, new line
// ACK: acknowledge            CR: carriage return
// BEL: bell                   SO: shift out
// DLE: data link escape       SI: shift in
// DC1: device control 1       EM: end of medium
// DC2: device control 2       FS: file separator
// DC3: device control 3       GS: group separator
// DC4: device control 4       RS: record separator
// NAK: negative acknowledge   US: unit separator
...
// ESC: escape
//
// 00  NUL   01  SOH   02  STX   03  ETX   04  EOT   05  QNQ   06  ACK   07  BEL
...
dc_i:
        ...
        case 'O': // O- open a empty line above
            ...
            case 'o':
                ...
                // Yes, I know it is in the middle of the "if (..."
                dot_end();
                dot = char_insert(dot, '\n');
            }
            else
...
// return pointer to NL of cur line line
static Byte *end_line(Byte *p)
{
    while(p < end - 1 && *p != '\n')
    {
...
static Byte *dollar_line(Byte *p)
{
    while(p < end - 1 && *p != '\n')
    {
        p++; // go to cur line E-o-l
...
static int count_lines(Byte *start, Byte *stop)
{
    Byte *q;
    int cnt;
...
// find begining of line #li
static Byte *find_line(int li)
{
    Byte *q;
...
static Byte *char_insert(Byte *p, Byte c)
{
    if(c == 22)
    {
        // Is this an ctrl-V?
...
static Byte *stupid_insert(Byte *p, Byte c)
{
...
//  open a hole in text[], at "p", make a 'size' byte hole
static Byte *text_hole_make(Byte *p, int size)
{
    Byte *src, *dest;
    int cnt;
...
//  close a hole in text[], delete "p" thru "q", inclusive
static Byte *text_hole_delete(Byte *p, Byte *q)
{
    Byte *src, *dest;
    int cnt, hole_size;
...
static void print_literal(Byte *buf, Byte *s)
{
    Byte c, b[2];
    b[1] = '\0';
...
// is the char a blank or tab
static int isblnk(Byte c)
{
    return (c == ' ' || c == '\t');
}
...
// IO Routines, read (maybe cursor) key from stdin
static Byte readit(void)
{
    Byte c;
...
#define ESCCMDS_COUNT (sizeof(esccmds)/sizeof(struct esc_cmds))
    ...
    if(bufsiz <= 0)
    {
ri0:    // the Q is empty, wait for a typed char
...
// get input line- use "status line"
static Byte *get_input_line(Byte *prompt)
{
    Byte buf[BUFSIZ];
    Byte c;
    int i;
...
static void place_cursor(int row, int col, int opti)
{
    ...
    // Try the standard terminal ESC sequence
    sprintf((char *) cm1, TES_CM, row + 1, col + 1);
    cm= cm1;
...
{
    write(1, TES_CEOL, strlen(TES_CEOL)); // Erase from cursor to end of line
}

// Erase from cursor to end of screen
static void clear_to_eos()
{
    write(1, TES_CEOS, strlen(TES_CEOS));
}

// Start standout mode, send "start reverse video" sequence
// Start reverse video mode
static void standout_start()
{
    write(1, TES_TSM_ON, strlen(TES_TSM_ON));
}

// End standout mode
// send "end reverse video" sequence
// End reverse video mode
static void standout_end()
{
    write(1, TES_TSM_OFF, strlen(TES_TSM_OFF));
}
...
static void beep()
{
    write(1, TES_BELL, strlen(TES_BELL)); // send out a bell character
}
...
static void psbs(char *format, ...)
{
    ...
    strcpy((char *) status_buffer, TES_TSM_ON); // Terminal standout mode on
    vsprintf((char *) status_buffer + strlen((char *) status_buffer), format, args);
    strcat((char *) status_buffer, TES_TSM_OFF); // Terminal standout mode off
    va_end(args);
    return;
}
